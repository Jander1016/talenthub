...
function TalentCard({ talent }: { talent: Talent}) {
  return (
      <div className="bg-white rounded-lg border-gray-800 mb-3 p-4 hover:bg-gray-100 hover: cursor pointer">
    
                <div className="grid grid-cols-1 gap-x-48 gap-y-20 md:grid-cols-2 lg:grid-cols-2 p-4 ">
                  {talent && talent.map((data: any) => (
                  <div key={data.id} className="group relative overflow-hidden rounded-sm bg-white shadow-one duration-700 hover:shadow-two dark:bg-dark dark:hover:shadow-gray-dark p-4">
