...
const TalentDetails = ({ params }) => {
 const [talent, setTalent] = useState<Talent | null>(null);

 useEffect(() => {
    const fetchTalent = async () => {
      try {
        const response = await fetch('http://localhost:3001/talents/', { cache: "force-cache" });
        if (!response.ok) {
          throw new Error('Error fetching talent');
        }
    ...
    fetchTalent();
 }, [params]); // Dependencia en params para refetch cuando cambie
 ...
 return (
    <div>
    {talent && 
    <h1>{talent.name_talent}</h1>}
      
      {/* Otros detalles del talento */}
    </div>
 );
};
